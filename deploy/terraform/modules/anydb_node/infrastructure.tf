...
resource "azurerm_subnet_network_security_group_association" "anydb" {
  ...
  network_security_group_id = var.infrastructure.vnets.sap.subnet_db.nsg.is_existing ? data.azurerm_network_security_group.anydb[0].id : azurerm_network_security_group.anydb[0].id
}
